...
5. **Gate enforcement rules (hard):**
   - **Additive-by-default:** every job declares upfront which tables/columns it may read and write. UPDATE only on declared columns; INSERT only into declared tables. Any proposal that overwrites a non-null value or deletes a row is a CONFLICT — shown in its own review section, never auto-applied.
   - **Immutable keys:** `row_id`s and company keys are permanent (money.csv/usecases.csv/vendors.csv join on them). The gate itself rejects any proposal touching a key — this is code, not reviewer vigilance.
   - **Dry-run cook:** before apply, run the cook against staged data in a temp dir and diff the resulting atlas stats (deployment counts, verdicts, entity counts) vs current. A job that changes stats outside its declared scope (e.g., size enrichment altering deployment counts) fails the dry run and cannot be applied.
   - **Batched apply:** large proposals apply in chunks (per-country or ≤200 rows), one git commit per applied batch — a bad batch is a one-command revert, never an archaeology dig.
...
2. Tagging pass — keyword/regex rules first, then ONE build-time Claude API pass on the residue (runs once in the pipeline, never at site runtime); all outputs committed to `data/usecases.csv` `{row_id, pattern_id, confidence, source_phrase, method: regex|llm}`; taxonomy in `data/usecase_taxonomy.csv`. LLM-tagged rows are reviewed through the Step 6 gate before merge.
...
**Do:**
1. Curated vendor dictionary (~200 names; propose seed in your plan) + regex pass; residue (rows with vendor-like phrasing but no dictionary hit) gets ONE build-time Claude API pass, reviewed through the Step 6 gate → `data/vendors.csv` `{row_id, vendor, vendor_type: model|cloud|platform|integrator|inhouse, source_phrase, method: regex|llm}`. **`inhouse` is a value** (build-vs-buy), "not disclosed" is a legitimate frequent outcome — never infer.
...
**The resweep doctrine (governs all three workloads — encode in the agent scripts, not just prompts):**
- **The register is an asset, not a cache.** The original census cost hundreds of search-hours. No job may re-derive, re-search, or regenerate what already exists. A "resweep" adds and refreshes — it never re-runs the census.
- **Skip-if-filled:** enrichment queries only null cells. A company with mktcap already present is skipped, not re-fetched.
- **Resumable, always:** any multi-row run writes a checkpoint (progress file per batch). A crash at row 1,800 resumes at 1,801. A run that restarts from zero is a bug.
- **Scope lock:** each workload's agent prompt states its read/write contract (matching the Step 6 declaration) and explicitly forbids drift ("while checking size, do NOT re-verify deployments").
- **Delta queries for freshness:** re-verification asks "what is new about {company} AI SINCE {last_verified}" — never "what AI does {company} have" from zero.
- **Freshness is a flow, not a one-shot:** a priority queue (oldest `last_verified` first, weighted by row visibility — value-number rows and high-traffic entities first) with a fixed budget per run (e.g., ~100 rows). The queue cycles forever; it never "finishes". This same queue becomes the ongoing heartbeat's re-verification engine.
...
- Register = source of truth; every number traceable to a source or a documented rule; static-first. LLM policy: NEVER at site runtime (visitors get static files only); allowed as a one-time BUILD-TIME parser (dictionary/regex first, Claude pass on the residue, outputs committed to CSV with `method` column, reviewed via the gate).
- All data mutations from Step 6 onward go through the gate. No exceptions, including your own scripts. All data jobs obey the resweep doctrine (Step 12): never re-derive existing data, skip-if-filled, resumable, scope-locked, delta queries only.
