...
.vfnhead {{ display:flex; align-items:center; justify-content:space-between; gap:16px; flex-wrap:wrap; margin:34px 0 0; }}
...
.open-fn .lbl {{ overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }}
...
table.grid th.row .rowlink {{ color:inherit; text-decoration:none; border-bottom:1px solid transparent; transition:color .15s, border-color .15s; }}
table.grid th.row .rowlink:hover {{ color:var(--accent); border-bottom-color:var(--accent); }}
...
<div class="topbar">
  ...
  <div class="topnav">
    ...
    <a href="#/industries" class="navlink" data-route="industries">Industries</a>
...
<!-- industries index (landing page for all verticals) -->
<section class="altitude" id="industries" data-alt="Industries">
  <div class="terr" id="industriesBody"></div>
</section>
...
const ROUTES = {{
  ...
  'industries':{{hash:'/industries',label:'Industries',show:renderVerticals}},
...
function applyRoute() {{
  ...
  if(h.startsWith('/vertical/')) {{ renderVertical(decodeURIComponent(h.slice('/vertical/'.length).split('?')[0]));
    document.querySelectorAll('.navlink').forEach(a=>a.classList.toggle('on', a.dataset.route==='industries')); return; }}
...
function renderVerticals() {{
  goAltitude('industries','Industries'); indexCompanies();
  const host=document.getElementById('industriesBody');
  const tot={{}}; (ATLAS.vert_totals_global||[]).forEach(t=>tot[t.v]=t);
  const cocount={{}}; ATLAS.companies.forEach(c=>{{ if(!c.silent) cocount[c.vertical]=(cocount[c.vertical]||0)+1; }});
  const openByV={{}}; const filled=new Set((ATLAS.grid_global||[]).map(c=>c.v+'|'+c.h));
  (ATLAS.verticals||[]).forEach(v=>{{ openByV[v]=(ATLAS.horizontals||[]).filter(h=>!filled.has(v+'|'+h)).length; }});
  const list=[...(ATLAS.verticals||[])].sort((a,b)=>(tot[b]?.n||0)-(tot[a]?.n||0));
  const cards=list.map(v=>{{
    ...
    return `<a class="ind-card" href="#/vertical/${{vslug(v)}}">
      ...
      ${{open?`<div class="ind-open">${{open}} open function${{open===1?'':'s'}}</div>`:''}}</a>`;
  }}).join('');
  host.innerHTML=`<h2 style="margin:0">Industries <span style="font-size:14px;color:var(--muted)">· ${{list.length}}</span></h2>
    <p class="lede" style="margin-top:6px">Every industry in the atlas, ranked by AI deployments found. Click any one for its per-function breakdown, open territory, and the companies behind it.</p>
...
function drawVertical(v, cc, slug, ccOpts) {{
  ...
  scope.innerHTML=`
    ...
    <div class="vfnhead">
      <h3 class="csub" style="margin:0">By business function</h3>
      <select id="vertCC" class="filtersel">${{ccOpts}}</select>
    </div>
  ...
  if(sel){{ sel.value=cc; sel.onchange=()=>{{ drawVertical(v, sel.value, slug, ccOpts); syncVertUrl(slug, sel.value); }}; }}
}}
...
function renderGrid() {{
  ...
  activeV.forEach((v,ri)=>{{
    html += `<tr><th class="row" data-ri="${{ri}}" title="${{v}} — open industry page"><a class="rowlink" href="#/vertical/${{vslug(v)}}">${{v}}</a></th>`;
    H.forEach((h,ci)=>{{
      const c = cells[v+'|'+h];
